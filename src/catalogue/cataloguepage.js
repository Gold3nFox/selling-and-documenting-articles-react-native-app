...
// import {Text, Image, BackHandler,TouchableOpacity,ActivityIndicator,Dimensions,StyleSheet,Platform,} from 'react-native';
...
//               this.setState({imageindex:pickedValue-1,isImageViewVisible: true});
//               this.forceUpdate();
//           },
...
// }
// getheight(){
//   if(this.state.response[window.mypagenumber].product_id != undefined){
//     return 50;
//   }else{
//     return 0;
//   }
...
//               <Text style={{fontSize:17,color:"white",}}>{parseInt(window.mypagenumber)+1} / {this.state.maxpage}</Text>
//           </TouchableOpacity>
//           </View>
//           <Button transparent disabled={this.state.response[window.mypagenumber].product_id != undefined ? false : true} style={{height:40,width:40,position: 'absolute',left:0}} onPress={()=>{this.setState({isImageViewVisible : false}); this.props.navigation.navigate('ProductSeries',{name : this.state.response[this.state.imageindex].name,route:'cataloguepage',catid:this.props.navigation.getParam('catalogue_id'),id : this.state.response[this.state.imageindex].product_id,family : this.state.response[this.state.imageindex].family, imageURL : this.state.response[this.state.imageindex].fimageURL,index:this.state.imageindex})}} >
...
//       }, 2000);
//       //
//       fetch(API_URL + '/auth/getCataloguePagenew?catalogue_id=' + this.props.navigation.getParam('catalogue_id')
//       , {
//         method: 'POST',
//         headers: {
...
//       this.setState({imageIndex:this.props.navigation.getParam('index')});
//     }
...
getwidth(){
  if(this.state.response != undefined && this.state.response[this.state.index].product_id != undefined){
    ...
    fetch_cataloguepages() {
      ...
      //
      fetch(API_URL + '/auth/getCataloguePagenew2?catalogue_id=' + this.props.navigation.getParam('catalogue_id') + '&width=' + Dimensions.get('window').width + '&height=' + Dimensions.get('window').height
      , {
        method: 'POST',
        headers: {
        Accept: 'application/json',
                 ...
                 <Modal visible={this.state.isImageViewVisible} transparent={true} onRequestClose={() => this.setState({ modalVisible: false })}>
                    <ImageViewer index={this.state.index} imageUrls={this.getimage()} onSwipeDown={() => {
                       this.setState({isImageViewVisible:false});  this.props.navigation.navigate('catalogue');
                        ...
                        enableSwipeDown={true} saveToLocalByLongPress={false} renderFooter={this.renderFooter} onChange={(index2)=>{this.setState({index:index2})}} />
                 </Modal>
